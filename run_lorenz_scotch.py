...
config.experiment_name = 'lorenz96' # 'lorenz96' or 'glycolysis'
...
if __name__ == '__main__':
    ...
    config.scotch_normalize = False
    config.scotch_solver_dt = 0.005
    config.scotch_noise_scale = 0.5
    ...
    # Fraction of observed time points (0.4 or 0.7)
    # (0.7 means p = 0.3, 30% missing, as in SCOTCH paper)
    # (0.4 means p = 0.6, 60% missing, as in SCOTCH paper)
    config.fraction = 0.4
    if config.fraction == 0.4:
        config.n_samples = 4000
        config.delta_t = 0.1  # Method solver step size
        config.train_steps = 2500
    elif config.fraction == 0.7:
        config.n_samples = 7000
        config.delta_t = 0.05  # Method solver step size
    ...
    results_file = 'results/lorenz96_scotch_settings.txt'
