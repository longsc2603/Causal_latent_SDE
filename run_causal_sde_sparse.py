...
parser = argparse.ArgumentParser(description='Configuration for Causal-SDE (sparse)')
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"Running sparse experiment: n_v={config.n_v}, n_z={config.n_z}, lr={config.lr}")
    print(f"{'='*80}")
    ...
    for k in range(num_runtimes):
        ...
        sparse_sampling_rate = int(config.obs_delta_t / config.delta_t)   # Sample every 5th time point
        ...
        with open(results_file, 'a') as f:
            ...
            f.write(f'method:Causal-SDE-sparse-{config.noise_type}-{config.n_z}-{config.n_v},')
